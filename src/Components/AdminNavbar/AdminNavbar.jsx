    ...
    return (
        <div>
            <div className="admin-navbar">
                ...
                <div onClick={()=>setOpenuser(!openUser)} style={{marginRight:'40px',display:'flex',alignItems:'center',cursor:'pointer'}}>
                <div style={{fontWeight:'600',fontSize:'14px',color:'#6B7280'}}>hai, {localStorage.getItem('user')}</div>
                <AiFillCaretDown style={{marginTop:'auto'}} color='#6B7280'/>
                </div>
                {
                    openUser && localStorage.getItem('role') =='superAdmin' &&   <div className="user-menu-container">
  <div className='user-menu-content' onClick={onAddUser} style={{borderBottom:'1px solid #FFFFFF',justifyContent:'center'}}><div>Add User</div></div>
  <div className='user-menu-content' onClick={onChangePassword} style={{justifyContent:'center'}}><div>Change Password</div></div>
                    </div>
                }
            <UserMenuDialog open={openForm} title={title} onClose={()=>setOpenForm(false)}/>
            ...
            <div className='admin-menu'>
                <ul >
                    ...
                    <li>
                    <Link style={{display:'flex',alignItems:'center',justifyContent:'space-between',width:'200px',color: location.pathname === '/adminpenjualan'?"#FDC232":""}} to='/adminpenjualan'>
