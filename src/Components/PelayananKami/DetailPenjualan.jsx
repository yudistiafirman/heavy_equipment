        ...
        <div className='detailPelatihanContainer'>
            ...
            <div className="detailTitleContainer">
              ...
              <AccordionSummary
                ...
                id="panel1a-header"
              >
                <Typography style={{fontSize:'18px',fontWeight:'800', color:'#6B7280',fontFamily:'"inter",sans-serif',lineHeight:'24.3px'}}>{v.title}</Typography>
              </AccordionSummary>
              <AccordionDetails>
            ...
            {
                dataAlat.length >0 && dataAlat.slice(0,5).map((v,i)=>{
                  return v.id != id &&    ( <div   onClick={()=>navigate('/penjualan/detailPenjualan/'+v.id+'/'+v.title_alatberat)}  className="lihatJugaContent">{cardTitleSlicer(v.title_alatberat)}</div>)
                })
              }
        </div>
        </div>
