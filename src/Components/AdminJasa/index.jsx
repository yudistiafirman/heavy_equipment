...
const AdminJasa = () => {
  ...
  const onDeleteList = () => {
    axios.delete(`${apiUrl}/jasa/delete?id=${selected}`).then(response => {
      if (response.data.error) {
        ...
        Swal.fire({
          icon: 'success',
          title: 'Penghapusan data Jasa Servis berhasil',
          text: 'Penghapusan  data Jasa Servis telah berhasil'
        })
        setSelected([])
        onGetFasilitasData('')
      }
              ...
              <TableBody>
                ...
                {rows.length > 0 &&
                  stableSort(rows, getComparator(order, orderBy))
                    ...
                    .filter((v,i)=>v.title.toLowerCase().includes(search.toLocaleLowerCase()))
                    .map((row, index) => {
