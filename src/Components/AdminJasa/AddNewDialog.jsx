import {
  ...
  const AddNewDialog = ({
    ...
    const onAddFasilitas=()=>{
      ...
      }else if (!kualifikasiForm[0].titleValue && !kualifikasiForm[0].descValue){
          alert('paling sedikit 1 nama scope services dan 1 deskripsi scope services harus diisi')
      }else{
          const data = {}
          const jasaToSend= []
          kualifikasiForm.map((v,i)=> {
              if(v.titleValue !== '' && v.descValue !== ''){
                  jasaToSend.push({scope_title:v.titleValue,scope_description:v.descValue })
              }
          })
  ...
  const onAddKualifikasi = (idx) => {
    ...
    if(kualifikasiForm[prevIdx].titleValue && kualifikasiForm[prevIdx].descValue){
      if (kualifikasiForm.length < 7) {
        SetKualifikasiForm([
          ...kualifikasiForm,
          { labelTitle: 'Nama Scope Services', titleValue: '',labelDesc:'Deskripsi',descValue:'' }
        ])
      } else {
      ...
      alert(`Nama scope services dan deskripsi ke ${kualifikasiForm.length} tidak boleh kosong`)
    }
